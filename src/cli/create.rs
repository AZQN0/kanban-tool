...
pub fn create(args: &CreateArgs) -> Result<()> {
    ...
    let priority = Priority::from_str(&args.priority)
        .ok_or_else(|| anyhow!("Unknown priority: {}. Use: backlog, low, medium, high, urgent", args.priority))?;
    ...
    // Write to SQLite
    let card_id = store.create_card(&card).context("Failed to create card in database")?;

    // Sync to markdown
    sync_card(&card, &cards_dir_path).context("Failed to write card markdown file")?;

    let col_name = board.columns.iter().find(|c| c.id == column_id).map(|c| c.name.as_str()).unwrap_or("?");
