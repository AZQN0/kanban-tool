...
fn get_board(app: &AppState) -> Result<Board, ApiError> {
    ...
    store
        .get_board(&app.project_path.to_string_lossy())
...
pub async fn list_boards(
    ...
    let db = config::db_path(&app.project_path);
    let store = Store::open(&db).map_err(|e| ApiError::internal(format!("DB error: {}", e)))?;
...
pub async fn create_card(
    ...
    // Broadcast SSE
    app.broadcaster
        .send(ServerSentEvent::card_created(&card))
        .ok();
...
pub async fn update_card(
    ...
    let new_col_id: Option<String> = if let Some(ref col_name) = body.column {
        let col = board
            .columns
            .iter()
            .find(|c| c.name == *col_name)
    ...
    let new_priority: Option<Priority> = if let Some(ref p) = body.priority {
        Some(
            Priority::from_str(p)
                .ok_or_else(|| ApiError::bad_request(format!("Invalid priority: {}", p)))?,
        )
    ...
    let updated = update_card_with_markdown(
        ...
        &config::cards_dir(&app.project_path),
    )
    .map_err(|e| store_error(e, "Failed to update card"))?;

    // Broadcast SSE
    app.broadcaster
        .send(ServerSentEvent::card_updated(&card_id))
        .ok();
...
pub async fn delete_card(
    ...
    delete_card_with_markdown(&mut store, &card_id, &config::cards_dir(&app.project_path))
        .map_err(|e| store_error(e, "Failed to delete card"))?;

    // Broadcast SSE
    app.broadcaster
        .send(ServerSentEvent::card_deleted(&card_id))
        .ok();
...
pub async fn move_card(
    ...
    // Broadcast SSE
    app.broadcaster
        .send(ServerSentEvent::card_moved(
            &card_id,
            &body.column,
            &target_col.name,
        ))
